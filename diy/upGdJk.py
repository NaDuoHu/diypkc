...
async def getNewVer():
    url = 'https://raw.githubusercontent.com/NaDuoHu/diypkc/main/bot/update.py'
...
async def upgdjk(event):
    try:
        ...
        async with jdbot.conversation(SENDER, timeout=60) as conv:
            msg = await conv.send_message(f"您是否更新要[pkc](https://github.com/NaDuoHu/diypkc.git)库的监控", buttons=btns, link_preview=False)
            ...
            if res == 'cancel':
                ...
                msg = await jdbot.edit_message(msg, f"好的，请稍等，正在检测版本...\n\n升级成功后会自动重启机器人。目录配置文件如有变动，请自行更新到/ql/config", link_preview=False)
                newversion = await getNewVer()
                if newversion:
                    if jk_version == newversion:
                        msg = await jdbot.edit_message(msg, f"当前版本:[{jk_version}](https://github.com/NaDuoHu/diypkc.git)\n最新版本:[{newversion}](https://github.com/NaDuoHu/diypkc.git)\n\n无需更新~", link_preview=False)
                        conv.cancel()
                        return
                    else:
