...
@client.on(events.NewMessage(pattern=r"^(user|在吗)(\?|\？)$"))
async def users(event):
    try:
        x = datetime.now().__format__('%Y-%m-%d')
        cz = open(f"/ql/config/tasklist/{str(x)}.txt", 'r')
        document = cz.readlines()
        document_num = int(len(document))
        await event.edit(f'船员你好，船长内部监控启动，杨帆起航\n青龙版本：`2.10.2`\n监控版本：`1.3.5`\n今日去重文件：`{str(x)}.txt`\n今日已跑变量：`{document_num}`\n配置变量: `{len(jk_list)}` | 当前监控: `{envNum}`')
        #dlmsg = await funCXDL()
...
@client.on(events.NewMessage(pattern=r"^监控明细$"))
...
async def activityID(event):
    try:
        ...
        for message in messages:
            ...
            ##---------程序最上面引入from datetime inport datetime-------##
            ##---------手动再configs目录下创建tasklist文件夹-------##
            totalkv = totalkv + kv + '\n'
            try:
                try:
                    data = open(f"/ql/config/tasklist/{str(x)}.txt",'r',encoding='utf-8')
                    gettasklist = data.read()
                    data.close()
                except:
                    try:
                        with open(f"/ql/config/tasklist/{str(x)}.txt" , 'w', encoding='utf-8') as f:
                            f.write('')
                    except:
                        pass
            except:
